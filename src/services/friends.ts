...
async function applyFriendRequestGating(user: SteamUser, id64: string): Promise<void> {
  let allow = false;
  try {
    allow = await hasActiveDeliveryForWinner(id64);
  } catch (err) {
    console.error(`[friends] DB error checking active deliveries for ${id64}:`, err);
    allow = false;
  }

  if (allow) {
    console.log(`[friends] Accepting friend request from ${id64} (active delivery exists)`);
    user.addFriend(id64);
    return;
  }

  console.log(`[friends] Declining friend request from ${id64}: no active delivery`);
